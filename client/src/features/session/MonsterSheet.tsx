          ...
          <div style={{ marginBottom: '0.85rem' }}>
            ...
            {effects.length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem', marginBottom: '0.4rem' }}>
                {effects.map((eff) => (
                  <div key={eff.name} style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', padding: '0.25rem 0.45rem', background: '#f9f0f0', border: '1px solid #e8d0d0', borderRadius: 4 }}>
                    <span style={{ flex: 1, fontSize: '0.78rem', fontWeight: 600, color: '#5a0000', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{eff.name}</span>
                    {eff.indefinite ? (
                      ...
                      <>
                        ...
                        <span style={{ minWidth: 30, textAlign: 'center', fontWeight: 700, fontSize: '0.75rem', color: eff.rounds <= 2 ? '#c44' : '#5a0000' }}>{eff.rounds}r</span>
                        <button onClick={() => socket.emit('token:effect_adjust', { token_id: tokenId, name: eff.name, delta: 1 })}
                          style={{ width: 20, height: 20, padding: 0, fontSize: '0.78rem', cursor: 'pointer', border: '1px solid #c88', borderRadius: 3, background: '#fff' }}>+</button>
                      </>
                    )}
